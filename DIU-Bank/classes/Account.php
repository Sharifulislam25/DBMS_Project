...
class Account {
    ...
    // Get all accounts with user info (for admin)
    ...
    // Get all accounts with search functionality (for admin)
    ...
    // Update balance (manual adjustment by admin)
    ...
    public function canDelete($account_id) {
        ...
        return [
            'can_soft_delete' => $balance == 0, // Can close if no balance
            'can_hard_delete' => ($balance == 0 && $transaction_count == 0), // Can delete if no balance and no history
            'balance' => $balance,
